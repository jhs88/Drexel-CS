...
$other_tables_file = "/home/student/pa4.1/CollegeScorecard_OtherTables.csv";
...
foreach ($csv as $k => $v) {
    if ($v['VARIABLE NAME'] <> '') {
        $other_tables[$v['VARIABLE NAME']]['COMMENT'] = substr($v['developer-friendly name'] . ": " . $v['NAME OF DATA
ELEMENT'], 0, 1024);
...
VARCHAR(1024),\n\t PRIMARY KEY (`" . $table . "_ID`)\n)";
        $create_other_tables = $create_other_tables . " ENGINE=InnoDB DEFAULT CHARSET=latin1
COMMENT='" . $attributes['COMMENT'] . "';";

        //Generate Inserts:
        $insert_other_tables = $insert_other_tables . "\n INSERT INTO `" . strtolower($table) . "`
(`" . $table . "_ID`,`" . $table . "_VALUE`) VALUES ";
...
file_put_contents('/home/student/pa4.1/create_other_tables.sql', $create_other_tables);
file_put_contents('/home/student/pa4.1/insert_other_tables.sql', $insert_other_tables);

//
// Create the main tables from catagoties:
//

$csv_file = "/home/student/pa4.1/CollegeScorecard_DataDictionary.csv";
...
'" . substr(str_replace("'", "", $attributes['COMMENT']), 0, 1024) . "' REFERENCES
`" . strtolower($column) . "` (`" . $column . "_ID`),";
                } else {
                    $create_schema = $create_schema . "\n\t`" . $column . "` " . $datatype . " COMMENT
'" . substr(str_replace("'", "", $attributes['COMMENT']), 0, 1024) . "',";
                }
            }
            $cols++;
        }
...
file_put_contents('/home/student/pa4.1/create_tables.sql', $create_schema);
...
$csv_file = "/home/student/pa4.1/MERGED2016_17_PP.csv";
                ...
                if (trim($table) <> '' && trim($table) <> 'completion') {
                    ...
                    foreach ($columns as $column => $attributes) {
                        if (trim($column) <> '' && trim($column) <> 'OPEID') {
                            ...
                            if (
                                $csv_row[$column] == 'NULL' || $csv_row[$column] == 'PrivacySuppressed' ||
                                $csv_row[$column] == ''
                            ) {
                                ...
                                $values_list =
                                    $values_list . "'" . mysqli_real_escape_string($conn, $csv_row[$column]) . "',";
                            }
                        }
                    }
